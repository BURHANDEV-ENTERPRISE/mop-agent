...
- **Fasa 6 (partial)** — ✅ execution backends `host`/`docker`/`ssh` for `run_shell` + `edit_code` (opt-in via `.MOP/link.json` capabilities + `execution`; capability+approval gated, path-escape guard, per-cmd timeout; docker `network:none` default). smoke-exec PASS. **Deferred:** voice (hardware/UI), more channels (Slack/WA/Signal).
- **Fasa 7 (partial)** — ✅ team foundation: roles (owner/member, `app_role`), **invite-gated email-scoped signup** (first user = owner; others need an invite), `requireRole` authz, `/api/me` + `/api/invites` (owner), gated `/api/link/code` + `/api/providers` POST. smoke-team PASS. **Deferred:** Postgres option (needs pgvector swap of sqlite-vec), cloud sync (multi-instance), full route-gating sweep + team UI (invite/members page).
